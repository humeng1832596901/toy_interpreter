...
struct Parser
{
    ...
    Expression *parseHashLiteral()
    {
        ...
        for (; not peekTokenIs(RBRACE);)
        {
            ...
            if (not peekTokenIs(RBRACE) && not expectPeek(COMMA))
            { // 如果下一个token既不是}也不是,
                delete hExp;
                return nullptr;
            }
    ...
    Program ParseProgram()
    {
        ...
        for (; curToken.TokenType != END; nextToken())
        {
            ...
            if (!res)
                break;
            // if(dynamic_cast<LetStatement*>(res))
                // std::cout << "xx\n";
            program.Statements.push_back(res);
        }
        // if(dynamic_cast<LetStatement*>(program.Statements[0]))
                // std::cout << "xx\n";
        return program;
    }
...
Statement *Parser::parseLetStatement()
{
    ...
    if (not expectPeek(IDENT))
    { // expectPeek函数有对token进行移位
        // 报错
        delete stmtPtr;
        return nullptr;
    }
    ...
    // std::cout << stmtPtr->Name.String() << ": " << stmtPtr->Value->String() << std::endl;
    return stmtPtr;
}
